            ...
            switch(k)
                ...
                case 4 % nweighting
                    ...
                    if ~isscalar(beta) && ~betaIsHandle
                        error(message('bioinfo:rankfeatures:NotValidBeta'))
                    end
                    if betaIsHandle
                        if any(beta(1:numPoints)<0)
                            error(message('bioinfo:rankfeatures:NotValidBetaFunctionHandle'))
                        end
                    else
                        if beta<0
                           error(message('bioinfo:rankfeatures:NegativeBeta'))
                        end
                    end
...
switch method
    ...
    case 'wilcoxon'
        ...
        z = zeros(numPoints,1);
        for i = 1:numPoints
            [~,~,stats] = ranksum(X(i,group)',X(i,~group)');
            z(i) = stats.ranksum;
        end
        z = abs(zscore(z));
    case 'roc'
