...
if nargin == 0
    if nargout 
        error(message('bioinfo:classperf:TooManyOutputArguments'))
    end
    displayProperties
    return
end
...
if ~isequal(class(CP),'biolearning.classperformance')
    ...
    if iscell(gT)
        if any(~cellfun('isclass',gT,'char'))
            error(message('bioinfo:classperf:InvalidCellForGT'))
        end
        if ismember('',gT)
            error(message('bioinfo:classperf:EmptyCellForGT'))
        end
    ...
    elseif isnumeric(gT) || islogical(gT)
        ...
        if ~isreal(gT) || any(isnan(gT)) || any(rem(gT,1)) 
            error(message('bioinfo:classperf:InvalidNumericForGT'))
        end
        [validGroups,~,igT] = unique(gT); 
    else
        error(message('bioinfo:classperf:InvalidTypeForGT'))
    end
    if max(igT)<2
         error(message('bioinfo:classperf:InvalidGroupsForGT'))
    end
...
if ~ischar(varargin{1})
    ...
    if CP.IsClassLabelTypeNumeric
        ...
        if ~isnumeric(gps) || ~isreal(gps) || any(gps(~isnan(gps))<0) || any(rem(gps(~isnan(gps)),1))
            error(message('bioinfo:classperf:InvalidNumericForGRP'))
        end
        [~,gps] = ismember(gps,CP.ClassLabels);
    elseif iscell(gps)
        if any(~cellfun('isclass',gps(:),'char'))
            error(message('bioinfo:classperf:InvalidCellForGRP'))
        end
        [~,gps] = ismember(gps,CP.ClassLabels);
    elseif isnumeric(gps)
        if ~isreal(gps) || any(gps(~isnan(gps))<0) || any(rem(gps(~isnan(gps)),1))
            error(message('bioinfo:classperf:InvalidIndicesForGRP'))
        end
    else
        error(message('bioinfo:classperf:InvalidTypeForGRP'))
    end
    gpsWereGiven = true;
end
...
elseif gpsWereGiven
    if numel(gps)~=CP.NumberOfObservations
        error(message('bioinfo:classperf:IncorrectSizeForClassout'));        
    end
...
if nvarargin
    ...
    if rem(nvarargin,2)
        error(message('bioinfo:classperf:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for j=1:2:nvarargin
        ...
        if isempty(k)
            error(message('bioinfo:classperf:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:classperf:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1 % positiveLabels
                    ...
                    if CP.IsClassLabelTypeNumeric
                        ...
                        if any(~tf) || isempty(tf)
                            error(message('bioinfo:classperf:InvalidPositiveLabels'))
                        end
                        positiveLabels = loc;
                    else
                        if ischar(pval)
                        ...
                        if ~iscell(pval) || ~all(cellfun('isclass',pval,'char'))
                            error(message('bioinfo:classperf:InvalidTypePositiveLabels'))
                        end
                        [tf,loc] = ismember(pval,CP.ClassLabels);
                        if any(~tf) || isempty(tf)
                            error(message('bioinfo:classperf:InvalidPositiveLabels'))
                ...
                case 2 % negativeLabels
                    ...
                    if CP.IsClassLabelTypeNumeric
                        [tf,loc] = ismember(pval,CP.ClassLabels);
                        if any(~tf) || isempty(tf)
                            error(message('bioinfo:classperf:InvalidNegativeLabels'))
                        end
                        negativeLabels = loc;
                    else
                        if ischar(pval)
                        ...
                        if ~iscell(pval) || ~all(cellfun('isclass',pval,'char'))
                            error(message('bioinfo:classperf:InvalidTypeNegativeLabels'))
                        end
                        [tf,loc] = ismember(pval,CP.ClassLabels);
                        if any(~tf) || isempty(tf)
                            error(message('bioinfo:classperf:InvalidNegativeLabels'))
    ...
    if ~isempty(intersect(positiveLabels,negativeLabels))
        error(message('bioinfo:classperf:InvalidPositiveNegativeLabels'))
    else
...
if islogical(idx)
    if numel(idx)~=CP.NumberOfObservations
        error(message('bioinfo:classperf:SizeLogicalIndex'))
    end
    if sum(idx)~=numel(gps)
        error(message('bioinfo:classperf:InvalidLogicalIndex'))
    end
    idx = find(idx);
else % it is a vector of indices
    if numel(unique(idx))~=numel(idx) || max(idx)>CP.NumberOfObservations || min(idx)<0 || any(rem(idx,1)>0)
        error(message('bioinfo:classperf:InvalidNumericIndex'))
    end
    if numel(idx)~=numel(gps)
        error(message('bioinfo:classperf:SizeNumericIndex'))
    end
end
