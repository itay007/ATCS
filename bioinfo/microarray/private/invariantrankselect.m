...
% INVARIANTRANKSELECT  Helper function for INVARIANTSETNORM and MAIRANKNORM Rank
...
if numel(baseline) ~= np
    error(message('bioinfo:invariantrankselect:XYmustBeSameLength'));
end
...
elseif tdLIM(1) < 0 || tdLIM(1) > 1  % threld must be between 0 and 1
    error(message('bioinfo:invariantrankselect:InvalidLowLimits'));
elseif tdLIM(2) < 0 || tdLIM(2) > 1  % threld must be between 0 and 1
    error(message('bioinfo:invariantrankselect:InvalidHighLimits'));
end 
...
elseif stopPCN < 0 || stopPCN > 100  % stop percent must be between 0 and 100
    error(message('bioinfo:invariantrankselect:InvalidStopPercent'));
end
...
elseif ~isnumeric(L) || L < 0 || L > np  
    error(message('bioinfo:invariantrankselect:InvalidRank'));
end
