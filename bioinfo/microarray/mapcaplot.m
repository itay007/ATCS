...
%
%   MAPCAPLOT(DATA) creates 2D scatter plots of the principal component
%   scores of DATA.  The scores used for the x and y data are selected from
%   popup menus, below each scatter plot. DATA can be a numeric matrix or a
%   DataMatrix object.
%
%   Once the scores have been plotted, a region can be selected in either
%   axes with the mouse.  This will highlight the points in the selected
%   region, and the corresponding points in the other axes.  This will also
%   display a list of the row numbers of the selected points in the
%   listbox.  Selecting an entry in the listbox will display a label with
%   the row number in each axes, at the corresponding point.  Clicking on a
%   point in the scatter plot will display a label with its row number
%   until the mouse is released.
%
%   MAPCAPLOT(DATA,LABELS) uses the elements of the cell array of strings
%   LABELS, instead of the row numbers, to label the data points.
%
%   Example:
%
...
%
%   See also CLUSTERGRAM, PCA, MATTEST, MAVOLCANOPLOT, YEASTDEMO.
...
elseif nargin>0
    ...
    if size(varargin{1},1) < 4
        error(message('bioinfo:mapcaplot:NotEnoughRows'))
    end
    ...
    error(message('bioinfo:mapcaplot:BadInputs'))
end
...
if size(varargin{1},2) < 3
    error(message('bioinfo:mapcaplot:NotEnoughColumns'))
end

% Assign data and labels
if isa(varargin{1}, 'bioma.data.DataMatrix')
    appdata.originaldata = varargin{1}.(':')(':');
else
    appdata.originaldata = varargin{1};
end
if size(appdata.originaldata,2) > size(appdata.originaldata,1)
    [appdata.pc,appdata.score,appdata.latent] = pca(appdata.originaldata,'econ',true);
else
    [appdata.pc,appdata.score,appdata.latent] = pca(appdata.originaldata,'econ',false);
end
...
if length(varargin)>1
    ...
    if length(labels) < size(appdata.originaldata,1)
        error(message('bioinfo:mapcaplot:LabelRowMismatch'))
    end
    ...
    labels = [];
    if isa(varargin{1}, 'bioma.data.DataMatrix')
        labels = get(varargin{1}, 'RowNames');
    end
...
%--------------------------------------------------------%
function localPopupmenuCallback(h,eventdata)  %#ok<*INUSD>
...
%-----------------------------------------------%
function localChangeCursorMotionFcn(obj,evd,fig)  %#ok<*INUSL>
obj = hittest(fig);
...
localFlipTextIfNeeded(t, [xlim ylim]);
p = line('Parent',ax,...
    'XData',xval,...
    'YData',yval,...
    'Color','white',...
...
%-----------------------------------------------%
function localRemoveLabelButtonUpFcn(obj,evd,fig,t,p) 
...
%-----------------------------------------------%
function localSelectedListCallback 
fig = gcbf;
...
for i = 1:length(selected)
    ...
    localFlipTextIfNeeded(t(i,1), [x_lim_ax1 y_lim_ax1]);
    ...
    t(i,2) = text('Parent',appdata.Axes2,...
        ...
        'Interpreter', 'none', ...
        'ButtonDownFcn','mapcaplot localDeleteMatching');
    localFlipTextIfNeeded(t(i,2), [x_lim_ax2 y_lim_ax2]);
...
function localExportSelectedList(varargin)  %#ok<DEFNU>
[~,hfig] = gcbo; 
...
labels = {'Selected Indices', 'Selected List'};
varnames = {'selindices', 'sellist'};

selindices = appdata.SelectedIndices;
sellist = appdata.rowlabels(selindices);

items = {selindices, sellist};
...
%-----------------------------------------------%
function localDeleteMatching  %#ok<DEFNU>
fig = gcbf;
appdata = localGetAppData(fig);

t = gco;

r = find(appdata.SelectedText == t,1);
...
%-----------------------------------------------%
function localFlipTextIfNeeded(textHandle, axisLimits)
extent = get(textHandle, 'Extent');
textSize = [extent(1) extent(1)+extent(3) extent(2) extent(2)+extent(4)];
if textSize(2)>axisLimits(2)
    set(textHandle, 'HorizontalAlignment', 'Right');
end
if textSize(4)>axisLimits(4)
    set(textHandle, 'VerticalAlignment', 'Top');
end
if textSize(3)<axisLimits(3)
    set(textHandle, 'VerticalAlignment', 'Bottom');
end
