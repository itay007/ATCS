...
if fid == -1
    error(message('bioinfo:ilmnbsread:CannotOpenFile', filename, fopenMessage));
end
...
if numHeaderLines >= maxHeaderLines || isempty(colNames)
    fclose(fid);
    error(message('bioinfo:ilmnbsread:NoTargetIDColumn', filename));
end
...
if ~allColumns
    ...
    if any(~foundCols)
        ...
        for count = notFound
            warning(message('bioinfo:ilmnbsread:ColumnNotFound', colNamesToRead{ count }));
        end
    end
...
if rem(nargin,2)== 1
    error(message('bioinfo:ilmnbsread:IncorrectNumberOfArguments', mfilename));
end
    ...
    switch(k)
        case 1  % columns
            ...
            if iscellstr(pval)
                ...
                error(message('bioinfo:ilmnbsread:ColumnsMustBeCellstr'));
            end
        case 2  % headeronly
