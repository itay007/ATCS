...
try
    fopenMessage = '';
    [fid, fopenMessage] = fopen(filename,'rt','n','ISO_8859-1');
    % Encoding is forced because the column headers of the GenePix format
    % have the character char(178)
catch theException %#ok<NASGU>
    fid = -1;
end

if fid == -1
    error(message('bioinfo:gprread:CannotOpenGPRFile', filename, fopenMessage));
end
...
if nargin > 1
    if rem(nargin,2) == 0
        error(message('bioinfo:gprread:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for j=1:2:nargin-2
        ...
        if isempty(k)
            error(message('bioinfo:gprread:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:gprread:AmbiguousParameterName', pname));
        else
            switch(k)
...
if ~strncmp(checkHeader,'ATF',3)
    fclose(fid);
    error(message('bioinfo:gprread:BadGPRFile'))
end
...
if isempty(nameCol) || isempty(IDCol)|| (nameCol ~=4 && IDCol ~=5)
    fclose(fid);
    error(message('bioinfo:gprread:ProblemsReadingGPR', filename));
end
...
for count = 1:numRows
    ...
    try
        ...
        warning(message('bioinfo:gprread:GPRBadLine', line));
    end
end
...
if (topLeft(1,1) <= bottomRight(1,1)) && (topLeft(1,2) <= bottomRight(1,2))
    ...
    warning(message('bioinfo:gprread:CannotDetermineGPROrientation'));
    fullIndices = [];
    return;
end
