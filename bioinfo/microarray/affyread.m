...
if ~ischar(filename)
    error(message('bioinfo:affyread:FilenameNotChar'));
end
...
if ~exist(fullpath,'file')
    error(message('bioinfo:affyread:FileDoesNotExist', theFile));
end
        ...
        try
            ...
            if(strcmpi(theExt,'.chp'))
                ...
                if exist(cdfFile,'file')
                    ...
                    warning(message('bioinfo:affyread:CHPNoCDF', affystruct.ChipType));
                end
            end
            
        catch theException
            if ~isempty(strfind(theException.identifier,'ReadError'))
                error(message('bioinfo:affyread:AffymetrixAPIMexMemError', theFile, theException.message))
            else
                error(message('bioinfo:affyread:AffymetrixAPIMexError', theFile, theException.message))
            end
        end
    otherwise
        error(message('bioinfo:affyread:UnknownFileType', theFile));
end
if isempty(affystruct)
    error(message('bioinfo:affyread:AffymexFailed', theFile));
end
