...
if nargin > 1
    lowercase = false;
    if rem(nargin,2)== 0
        error(message('bioinfo:int2aa:IncorrectNumberOfArguments', mfilename));
    end
    okargs = {'case',''};
    for j=1:2:nargin-2
        pname = varargin{j};
        ...
        if isempty(k)
            error(message('bioinfo:int2aa:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:int2aa:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1  % case
