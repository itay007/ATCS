...
%   cannot pass through the 'joining' segment between the 'End' and the
%   'Begin' states.
%
%   Example:
%
...
if nargin > 1
    if rem(nargin,2) == 0
        error(message('bioinfo:hmmprofgenerate:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for j=1:2:nargin-2
        ...
        if isempty(k)
            error(message('bioinfo:hmmprofgenerate:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:hmmprofgenerate:AmbiguousParameterName', pname));
        else
            switch(k)
