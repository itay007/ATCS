...
%   such states did not emit a symbol in the aligned sequence, either
...
%
%   See also GETHMMPROF, HMMPROFDEMO, HMMPROFESTIMATE, HMMPROFGENERATE,
%   HMMPROFMERGE, HMMPROFSTRUCT, MULTIALIGN, PFAMHMMREAD, PROFALIGN,
%   SHOWHMMPROF.
...
if isstruct(seq)
    ...
    if isempty(seq) || (iscell(seq)&&all(cellfun('isempty',seq)))
        error(message('bioinfo:hmmprofalign:SequenceNotFoundInStructure'));
    end
end

if iscell(seq) 
    if numel(seq)>1
            warning(message('bioinfo:hmmprofalign:NoMultipleSequences'))
    end
    seq = seq{1};
end

if ~ischar(seq) || ~isvector(seq)
    error(message('bioinfo:hmmprofalign:IncorrectInputSequence'))
end
...
if nargin > 2
    if rem(nargin,2) == 1
        error(message('bioinfo:hmmprofalign:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for j=1:2:nargin-2
        ...
        if isempty(k)
            error(message('bioinfo:hmmprofalign:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:hmmprofalign:AmbiguousParameterName', pname));
        else
            logpval = bioinfoprivate.opttf(pval);
            if isempty(logpval)
                 error(message('bioinfo:hmmprofalign:InputOptionNotLogical', upper( okargs{ k } )));
            end
            switch(k)
...
if any(intseq > alphaLength) || any(~intseq)
   error(message('bioinfo:hmmprofalign:SymbolsNotDefined'));
end
