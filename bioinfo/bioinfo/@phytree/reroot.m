...
if numel(tr)~=1
     error(message('bioinfo:phytree:reroot:NoMultielementArrays'));
end
...
if nargin == 1;
    ...
    if islogical(node)
        ...
        elseif numel(node) == numBranches
            ...
            error(message('bioinfo:phytree:reroot:IncorrectSizeInputVector'));
        end
    end
    if ~isscalar(node) || node<1 || node>numLabels
        error(message('bioinfo:phytree:reroot:InvalidInputNode'));
    end
    node = round(node);
...
if ~isscalar(distance) || distance<0
    error(message('bioinfo:phytree:reroot:InvalidInputDistance'));
end
...
if node == numLabels 
    if distance>0
        warning(message('bioinfo:phytree:reroot:BeyondRoot'))
    else
        tr.names{end} = 'Root';
    end
    return
...
h = tr.dist~=0; % root cannot be in an edge which length is zero
h(numLabels) = false; % the route cannot be segmented
x = inf(numLabels,1);
