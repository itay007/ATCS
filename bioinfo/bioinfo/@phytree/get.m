...
if numel(tr)~=1
     error(message('bioinfo:phytree:get:NoMultielementArrays'));
end
...
for ind = 2 : nargin
    ...
    if isempty(k)
        error(message('bioinfo:phytree:get:UnknownParameterName', pname));
    elseif length(k)>1
        error(message('bioinfo:phytree:get:AmbiguousParameterName', pname));
    else
        switch(k)
            case 1 % pointers
