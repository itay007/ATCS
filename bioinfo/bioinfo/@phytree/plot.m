...
%       set(h.leafNodeLabels,'FontSize',6,'Color',[.2 .5 .7])
%
%   See also PHYTREE, PHYTREE/VIEW, PHYTREEREAD, PHYTREEVIEWER,
%   SEQNEIGHJOIN, SEQLINKAGE. 

% Copyright 2003-2012 The MathWorks, Inc.


if numel(tr)~=1
    error(message('bioinfo:phytree:plot:NoMultielementArrays'));
end
...
tr.circularType = circularType;
tr.dispBranchLabels = dispBranchLabels;
tr.dispLeafLabels = dispLeafLabels;
tr.dispTerminalLabels = dispTerminalLabels;
tr.rotateLeafLabels = rotateLeafLabels;

if nargin - argStart > 0
    if rem(nargin - argStart,2) == 1
        error(message('bioinfo:phytree:plot:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for j = argStart:2:nargin-argStart
        ...
        if isempty(k)
            error(message('bioinfo:phytree:plot:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:phytree:plot:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1 % type
                    oktypes={'square','angular','radial',...
                             'equalangle','equaldaylight'};
                    l = strncmpi(pval,oktypes,numel(pval)); 
                    if sum(l)==0
                        error(message('bioinfo:phytree:plot:UnknownTypeName', upper( okargs{ k } )));
                    elseif sum(l)>1
                        error(message('bioinfo:phytree:plot:AmbiguousTypeName', upper( oktypes{ l } )));
                    else
                        tr.renderType = oktypes{l};
                        if l>2
                            tr.circularType = true;
                        end
                    end
                case 2 % orientation
                    ...
                    l = strncmpi(pval,oktypes,numel(pval)); 
                    if sum(l)==0
                        error(message('bioinfo:phytree:plot:UnknownOrientation', upper( okargs{ k } )));
                    else
                        tr.orientation = oktypes{l};
                    end
                case 3 % rotation
                    if isreal(pval(1))
                        tr.rotation = double(pval(1))*pi/180;
                    else
                        error(message('bioinfo:phytree:plot:NotValidType'));
                    end
                case 4 % branch labels
                    tr.dispBranchLabels = bioinfoprivate.opttf(pval);
                case 5 % leaf labels
                    tr.dispLeafLabels = bioinfoprivate.opttf(pval);
                case 6 % terminal labels
                    tr.dispTerminalLabels = bioinfoprivate.opttf(pval);
                case 7 % rotate leaf labels
                    tr.rotateLeafLabels = bioinfoprivate.opttf(pval);
            end
        end
    end
end

% set dependent default for tr.dispLeafLabels
if isnan(tr.dispLeafLabels)
    if tr.circularType
        tr.dispLeafLabels = true;
    else
        tr.dispLeafLabels = false;
    end
end
% terminal labels are not allowed on circular layouts
if tr.circularType
    tr.dispTerminalLabels = false;
end
...
tr.prunedLeafIndex = setdiff(leafIndex,leafDisplayedAsBranch);
tr.prunedBranchIndex = [branchIndex leafDisplayedAsBranch];
...
switch tr.renderType
    case {'square','angular','radial'}
        ...
        if strcmp(tr.renderType,'angular')
    ...
    case{'equalangle', 'equaldaylight'}
        ...
        if strcmp(tr.renderType, 'equaldaylight')
            for selectedNode = branchIndex(end-1:-1:1)
%                 if selectedNode<=553
%                     createFigure(tr)
%                     hold on
%                     plot(tr.x(selectedNode),tr.y(selectedNode),'ro')
%                 end
                ...
                x0 = tr.x(selectedNode);
                x1 = tr.x(tr.tree(selectedNode - tr.numLeaves, 1));
                x2 = tr.x(tr.tree(selectedNode - tr.numLeaves, 2));
                x3 = tr.x(tr.par(selectedNode));
                y0 = tr.y(selectedNode);
                y1 = tr.y(tr.tree(selectedNode - tr.numLeaves, 1));
                y2 = tr.y(tr.tree(selectedNode - tr.numLeaves, 2));
                y3 = tr.y(tr.par(selectedNode));
                ...
                if ~((xdiff1==0 && ydiff1==0) || (xdiff2==0 && ydiff2==0) || (xdiff3==0 && ydiff3==0))  
                
                    refangles1 = atan2(ydiff1,xdiff1);
                    refangles2 = atan2(ydiff2,xdiff2);
                    refangles3 = atan2(ydiff3,xdiff3);
                                          
                    st1angles = atan2((tr.y(subtree1Leaf) - y0), (tr.x(subtree1Leaf) - x0)) - refangles1;
                    st2angles = atan2((tr.y(subtree2Leaf) - y0), (tr.x(subtree2Leaf) - x0)) - refangles2;
                    st3angles = atan2((tr.y(subtree3Leaf) - y0), (tr.x(subtree3Leaf) - x0)) - refangles3;
                    ...
                    [~,lineNodeIdx1] = max(st1angles);
                    [~,lineNodeIdx2] = min(st1angles);
                    [~,lineNodeIdx3] = max(st2angles);
                    [~,lineNodeIdx4] = min(st2angles);
                    [~,lineNodeIdx5] = max(st3angles);
                    [~,lineNodeIdx6] = min(st3angles);
                
                    A = mod([st1angles(lineNodeIdx1) + refangles1; ...
                             st1angles(lineNodeIdx2) + refangles1; ...
                             st2angles(lineNodeIdx3) + refangles2; ...
                             st2angles(lineNodeIdx4) + refangles2; ...
                             st3angles(lineNodeIdx5) + refangles3; ...
                             st3angles(lineNodeIdx6) + refangles3], 2*pi);
...
set(h.axes,'Tag',tr.orientation);
%set render type and rotation in appdata for testing purposes
setappdata(fig,'RenderType',tr.renderType);
setappdata(fig,'Rotation',tr.rotation*180/pi);
hold on;
...
switch tr.renderType
    ...
    case 'radial'
        ...
        tmp = [X(:), Y(:)] * [cos(tr.rotation) sin(tr.rotation); -sin(tr.rotation) cos(tr.rotation)];
        X = reshape(tmp(:,1), size(t_interp));
        Y = reshape(tmp(:,2), size(t_interp));
        
        tr.x = X(1,:)';
        tr.y = Y(1,:)';
        tr.ang = theta(1,:)' + tr.rotation;
        tr.tang = theta(1,:)' + tr.rotation;
...
if ~tr.circularType
    switch tr.orientation
        case {'left','right'}
            h.BranchDots = plot(tr.x(tr.prunedBranchIndex),tr.y(tr.prunedBranchIndex),'o',...
                'MarkerSize',5,'MarkerEdgeColor','k',...
                'MarkerFaceColor','b');
            h.LeafDots = plot(tr.x(tr.prunedLeafIndex),tr.y(tr.prunedLeafIndex),'square',...
                'MarkerSize',4,'MarkerEdgeColor','k',...
                'MarkerFaceColor','w');
        case {'top','bottom'}
            h.BranchDots = plot(tr.y(tr.prunedBranchIndex),tr.x(tr.prunedBranchIndex),'o',...
                'MarkerSize',5,'MarkerEdgeColor','k',...
                'MarkerFaceColor','b');
            h.LeafDots = plot(tr.y(tr.prunedLeafIndex),tr.x(tr.prunedLeafIndex),'square',...
                'MarkerSize',4,'MarkerEdgeColor','k',...
                'MarkerFaceColor','w');
    end
else
...
if ~tr.circularType && any(strcmp(tr.orientation,{'bottom','top'}))
    X = tr.y(tr.prunedBranchIndex);
    Y = tr.x(tr.prunedBranchIndex);
else % {'left', 'right'}
    X = tr.x(tr.prunedBranchIndex);
    Y = tr.y(tr.prunedBranchIndex);
end
h.branchNodeLabels = text(X,Y,tr.names(tr.prunedBranchIndex));
...
if ~tr.circularType
    switch tr.orientation
        case {'left','right'}
            ...
            set(h.branchNodeLabels(~ismember(tr.prunedBranchIndex,branchIndex)),'horizontal','left')
        case {'bottom','top'}
            set(h.branchNodeLabels,'vertical','bottom')
...
if ~tr.circularType && any(strcmp(tr.orientation,{'bottom','top'}))
    X = tr.y(tr.prunedLeafIndex);
    Y = tr.x(tr.prunedLeafIndex);
else % {'left', 'right'}
    X = tr.x(tr.prunedLeafIndex);
    Y = tr.y(tr.prunedLeafIndex);
end
h.leafNodeLabels = text(X,Y,tr.names(tr.prunedLeafIndex));
...
if ~tr.circularType
    ...
    hal = {'left', 'right'};
    val = {'top','bottom'};
    if tr.rotateLeafLabels && numel(tr.prunedLeafIndex)>0
        rot = tr.ang(tr.prunedLeafIndex)*180/pi;
        
        [~,k] = sort(tr.tang(tr.prunedLeafIndex)*180/pi);
        rot = rot(k);
...
if ~tr.circularType
    if any(strcmp(tr.orientation,{'bottom','top'}))
...
if any(strcmp(tr.orientation,{'left','right'}))
...
if any(strcmp(tr.orientation,{'left','right'}))
    ...
    axesPosition = [30 40 max(minextpix,reqextpix(1)) figurePosition(4)-70];
else % {'bottom','top'}
    if tr.dispTerminalLabels
    ...
    axesPosition = [60 30 figurePosition(3)-90 max(minextpix,reqextpix(2))];
end
