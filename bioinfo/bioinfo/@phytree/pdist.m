...
%   distances between every possible pair of leaf nodes of the phylogenic
...
%
%   See also SEQPDIST, SEQLINKAGE, PHYTREE, PHYTREEVIEWER.

% Copyright 2003-2012 The MathWorks, Inc.


if numel(tr)~=1
     error(message('bioinfo:phytree:pdist:NoMultielementArrays'));
end
...
if nargin > 1
    if rem(nargin,2) == 0
        error(message('bioinfo:phytree:pdist:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for ind = 2 : 2: nargin
        ...
        if isempty(k)
            error(message('bioinfo:phytree:pdist:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:phytree:pdist:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1 % nodes
                    ...
                    if isempty(h)
                       error(message('bioinfo:phytree:pdist:IncorrectReferenceNode'))
                    else 
                        outNodes = okNodes{h};
                    end
            case 2 % squareform
            ...
            case 3  % criteria
                   ...
                   else error(message('bioinfo:phytree:pdist:InvalidCriteria'));
                   end
            end
        end
    end
