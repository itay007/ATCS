...
%      view(tr)
%      
%   See also PHYTREE, PHYTREE/SELECT, PHYTREE/GET, PHYTREEVIEWER.
...
if numel(tr)~=1
     error(message('bioinfo:phytree:prune:NoMultielementArrays'));
end
...
if islogical(sel)
    ...
    elseif numel(sel)==numBranches
        ...
        error(message('bioinfo:phytree:prune:IncorrectLogical'));
    end
elseif isnumeric(sel) && isreal(sel) && all(sel>=1) && all(sel<=numLabels)
    ...
    error(message('bioinfo:phytree:prune:IncorrectTypeofArguments'));
end

nvarargin = numel(varargin);
if nvarargin
    if rem(nvarargin,2)
        error(message('bioinfo:phytree:prune:IncorrectNumberOfArguments', mfilename));
    end
    okargs = {'mode',''};
    for j=1:2:nvarargin
        pname = varargin{j};
        ...
        if isempty(k)
            error(message('bioinfo:phytree:prune:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:phytree:prune:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1 % classifiers
                    ...
                    if isempty(modeSelected) 
                      error(message('bioinfo:phytree:prune:NotValidMode'))
                    end
...
for ind = numBranches:-1:1
 ...
 if sel(numLabels) 
     warning(message('bioinfo:phytree:prune:PrunedRoot'))
     tr=btr; return
 end
...
if ~exclusiveMode % (the selected nodes are deleted with their descendants)
    ...
    if sum(~sel) == 1
            warning(message('bioinfo:phytree:prune:NotAMinimumTree'))
            tr=btr; return
    end
    ...
    % for each branch, propagate the smallest leaf index
    smallest = 1:numLabels;
    for ind = 1:numBranches
        smallest(numLeaves+ind) = min(smallest(tr.tree(ind,:)));
    end
    ...
    newLeaves = find([~sel(1:numLeaves);top(numLeaves+1:end)]);
    ...
    [~,h] = sort(smallest(newLeaves));
    permuta =  [newLeaves(h);numLeaves+find(stayingBranches)];
    ipermuta(permuta) = 1:length(permuta);
    % update all tree structure fields
    tr.names = tr.names(permuta);
    tr.dist = tr.dist(permuta);
