...
if numel(tr)~=1
     error(message('bioinfo:phytree:select:NoMultielementArrays'));
end
...
if nargin - first_arg + 1 > 0
   if rem(nargin - first_arg,2) == 0
       error(message('bioinfo:phytree:select:IncorrectNumberOfArguments', mfilename));
   end
   ...
   for j=first_arg - 1 : 2 : nargin - first_arg + 1
       ...
       if isempty(k)
           error(message('bioinfo:phytree:select:UnknownParameterName', pname));
       elseif length(k)>1
           error(message('bioinfo:phytree:select:AmbiguousParameterName', pname));
       else
           switch(k)
               case 1  % reference
                   if islogical(pval)
                       ...
                       elseif numel(pval)==numBranches
                         ...
                         error(message('bioinfo:phytree:select:InvalidSizeLogicalReferenceNode'))
                       end
                       pval = find(pval);
                       ReferenceIs = 'node';
                       if numel(pval) ~= 1 
                           error(message('bioinfo:phytree:select:InvalidValuesLogicalReferenceNode'))
                       else
                           ReferenceNode = pval;
                       end                       
                   elseif isnumeric(pval)
                       ReferenceIs = 'node';
                       if numel(pval) ~= 1 
                           error(message('bioinfo:phytree:select:InvalidSizeReferenceNode'))
                       elseif all(pval~=1:numLabels)
                           error(message('bioinfo:phytree:select:InvalidValueReferenceNode'))
                       else
                           ReferenceNode = pval;
                       end
                   else
                       ...
                       else error(message('bioinfo:phytree:select:InvalidStringReferenceNode'));
                       end
                   end
                case 2  % criteria
                   ...
                   else error(message('bioinfo:phytree:select:InvalidCriteria'));
                   end
               case 3 % threshold
                   V = pval;
                   if (~isnumeric(V) || ~isreal(V) || numel(V)>1)
                       error(message('bioinfo:phytree:select:InvalidThreshold'));  
                   end
               case 4 % exclude
                   ...
                   if numel(h)
                       ...
                       else error(message('bioinfo:phytree:select:InvalidExcludeOption'));
                   end
               case 5 % propagate
                   ...
                   if numel(h)
                       ...
                       else error(message('bioinfo:phytree:select:InvalidPostPropagate'));
                   end
           end % switch(k)
       end %  if ...
   end % for j=...
