function [ha, H] = rnaplot(s, varargin)
...
switch typePlot
    ...
    case 2 % diagram
        [ha, H] = localMatrix2diagram(s2, selected, seq, colorScheme, colors, 0);
    case 3 % dotdiagram
        [ha, H] = localMatrix2diagram(s2, selected, seq, colorScheme, colors, 1);
    case 4 % graph
...
text(labelRadius * cos(theta(ticks)), labelRadius * sin(theta(ticks)), tickLabels, ...
    'HorizontalAlignment', 'center'); % numbering
set(ha, 'yLim', [-1 1.1]);
%set(ha, 'xLim', [-1 1.1]);

%=== Datatips and callbacks
datatipLabels = localCreateDatatipLabels(N, seq);
ht = localCreateDatatips(hf, cos(theta), sin(theta), datatipLabels);
...
function [ha H] = localMatrix2diagram(matrix, selected,  seq,  colorScheme, colors, dot)
...
ht = localCreateDatatips(hf, x(2:end-1), y(2:end-1), datatipLabels);
...
N = size(matrix, 1);
datatipLabels = localCreateDatatipLabels(N, seq);
ht = localCreateDatatips(hf, 1:N, zeros(1,N), datatipLabels);
...
%=== datatips and callbacks
datatipLabels = localCreateDatatipLabels(N, seq);
ht = localCreateDatatips(hf, 1:N, mountain, datatipLabels);
...
ht = localCreateDatatips(hf, xy(:,1), xy(:,2), datatipLabels);
...
%==========================================================================
function localPlaceDataTip(dataTipText, dataTipHandle)
% Place datatip

set(dataTipHandle,'String',dataTipText,'visible','on')
%set(gcbf,'WindowButtonMotionFcn',{@(h,e,tip) localMoveDataTip(tip), dataTipHandle})
localMoveDataTip(dataTipHandle);

% =========================================================================
function localMoveDataTip(dataTipHandle)
% Move datatip -- NOT SUPPORTED for now

ha = gca;
set(dataTipHandle,'Position',[1 1]*get(ha,'CurrentPoint')/2)

%=========================================================================
function localDeleteDataTip(dataTipHandle)
% Delete data tip

set(gcbf,'WindowButtonMotionFcn',[])
set(dataTipHandle,'String','','visible','off')
...
%==========================================================================
function ht = localCreateDatatips(hf, x, y, labels)
% Create datatip in figure hf at positions given by coordinates x and y,
...
for k = 1:N
    ht(k) = plot(x(k), y(k), '.', 'MarkerEdgeColor', 'none', ...
        'MarkerFaceColor', 'none', 'UserData', labels{k});
end
...
set(ht,'ButtonDownFcn', {@(h,e, tip, a) localPlaceDataTip(get(h,'UserData'), tip), hTip});
set(hf,'WindowButtonUpFcn',{@(h,e, tip) localDeleteDataTip(tip), hTip});
...
if isdiagram
    q = text(x(z), y(z), seq(z), 'HorizontalAlignment', 'Center', ...
        'FontSize', mysize, 'Color', mycolor, 'Clipping', 'off');
else % dot
...
% plotted as non visible with coordinates diagCoord. Return handle to
...
if ~isempty(h1) % Paired | A | AU
    if isempty(diagCoord)
        ...
        hLeg(1) = plot(diagCoord(1), diagCoord(2), '.', 'Color', legendColors{1}, 'Visible', 'off');
    end
end
...
if ~isempty(h2) % Unpaired | C | GU
    if isempty(diagCoord)
        ...
        hLeg(2) = plot(diagCoord(1), diagCoord(2), '.', 'Color', legendColors{2}, 'Visible', 'off');
    end
end
...
if ~isempty(h3) % [] | G | GC
    if isempty(diagCoord)
        ...
        hLeg(3) = plot(diagCoord(1), diagCoord(2), '.', 'Color', legendColors{3}, 'Visible', 'off');
    end
end
...
if ~isempty(h4) % [] | U | Unpaired
    if isempty(diagCoord)
        ...
        hLeg(4) = plot(diagCoord(1), diagCoord(2), '.', 'Color', legendColors{4}, 'Visible', 'off');
    end
end
...
if ~isempty(h5) % Selected
    if isempty(diagCoord)
        ...
        hLeg(5) = plot(diagCoord(1), diagCoord(2), '.', 'Color', legendColors{5}, 'Visible', 'off');
    end
    if length(h5) > 1
        hnot = h5(2:end);
    end
