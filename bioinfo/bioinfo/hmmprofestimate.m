...
%
%   See also HMMPROFDEMO, HMMPROFSTRUCT, SHOWHMMPROF, HMMPROFALIGN
...
% check varargin
if rem(nargin,2) == 1
    error(message('bioinfo:hmmprofestimate:IncorrectNumberOfArguments', mfilename));
end
if nargin>2 && ~ischar([varargin{1:2:end}])
    error(message('bioinfo:hmmprofestimate:IncorrectParameterNames', mfilename));
end

%Default pseudocount constants
A=20; % for symbol emission, can be 5xR or other!
Ax=20; % for state transition

%Default background transitions
qabx=[0.01 0.99];
qadx=[.5 .5];
qaix=[.5 .5];
...
if isstruct(ma)
    ...
    if isempty(ma) || (iscell(ma)&&all(cellfun('isempty',ma)))
        error(message('bioinfo:hmmprofestimate:SequenceNotFoundInStructure'));
    end
end
...
if nargin > 2
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
        k = find(strcmpi(pname, parmNames));
        if numel(k) ~= 1
            k = strmatch(lower(pname), lower(parmNames));
        end
        if isempty(k)
            error(message('bioinfo:hmmprofestimate:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:hmmprofestimate:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1 % 'A' Pseudocount for Emission Probs.
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=1 || pval<0 || pval>1e9
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueA', pname))
                    end
                    A = pval;
                case 2 % 'Ax' Pseudocount for Tx Prob.
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=1 || pval<0 || pval>1e9
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueAx', pname))
                    end
                    Ax = pval;
                case 3 % 'BE' Background Symbol Emission
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=alphaLength || any(pval<0)
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueBE', pname))
                    end
                    qa = pval(:)'./sum(pval);
                case 4 % 'BMx' Background Math Tx Prob.
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=3 || any(pval<0)
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueBMx', pname))
                    end
                    qamx = [pval(:)' 0]./sum(pval);
                case 5 % 'BDx' Background Delete Tx Prob.
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=2 || any(pval<0)
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueBDx', pname))
                    end
                    qadx = pval(:)'./sum(pval);
                case 6 % 'BIx' Background Insert Tx Prob.
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=2 || any(pval<0)
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueBIx', pname))
                    end
                    qaix = pval(:)'./sum(pval);
                case 7 % 'BBx' Background Begin Tx Prob.
                    if ~isnumeric(pval) || ~isreal(pval) || numel(pval)~=2 || any(pval<0)
                        error(message('bioinfo:hmmprofestimate:IncorrectParameterValueBBx', pname))
                    end
                    qabx = pval(:)'./sum(pval);
            end
        end
...
if ~ischar(ma)
   error(message('bioinfo:hmmprofestimate:IncorrectInputType'))
end
...
if all(sum((ma>='A' & ma<='Z') | ma=='-',2)==profLength)
    ...
    [~,h]=sortrows([-sum(symCount);-entropy]');
    ptr=sort(h(1:profLength));
end

numSeq = size(ma,1);

%make spaces uniform before converting to ints
ma(:) = regexprep(ma(:)','[~\s\.]','-');
  
if (isamino && ~bioinfoprivate.isaa(ma)) || (~isamino && ~bioinfoprivate.isnt(ma))
   error(message('bioinfo:hmmprofestimate:IncorrectAlpha'))
end
