...
if nargin > 1
    if rem(nargin,2) == 0
        error(message('bioinfo:ntdensity:IncorrectNumberOfArguments', mfilename));
    end
    ...
    for j=1:2:nargin-2
        ...
        if isempty(k)
            error(message('bioinfo:ntdensity:UnknownParameterName', pname));
        elseif length(k)>1
            error(message('bioinfo:ntdensity:AmbiguousParameterName', pname));
        else
            switch(k)
                case 1   % window
                    window = pval;
...
% Pad the end of the sequence to accommodate the window size
