...
%
%   See also CAT, PAREN, SEQRCOMPLEMENT, STRCAT, STRFIND, RANDSEQ.
...
if isstruct(left)
    left = bioinfoprivate.seqfromstruct(left);
end

if isstruct(right)
    right = bioinfoprivate.seqfromstruct(right);
end
...
rightLength = findprefix(left(1:slen),right(rlen-slen+1:end),1);

leftLength = findprefix(right(1:slen),left(llen-slen+1:end),rightLength);
